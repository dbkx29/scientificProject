...
# ============= 图形风格与中文字体设置 =============
plt.style.use('seaborn-v0_8-darkgrid')  # 更美观的图形风格
...
def compute_stats(signal: np.ndarray) -> Dict[str,float]:
    if signal.size==0:
    ...
    krt_val=float(kurtosis(signal,fisher=True,bias=False))
    skw_val=float(skew(signal,bias=False))
    peak_val=float(np.max(np.abs(signal)))
    crest_val=float(peak_val/rms_val) if rms_val>0 else np.nan
    return dict(均值=mean_val,标准差=std_val,RMS=rms_val,峭度=krt_val,偏度=skw_val,峰值=peak_val,峰值因子=crest_val)

def maybe_downsample(signal: np.ndarray, fs: Optional[float], max_pts: Optional[int]):
    if not max_pts or signal.size<=max_pts: return signal, fs
    step=int(math.ceil(signal.size/max_pts))
    return signal[::step], (fs/step if fs else fs)
...
# ============= 绘图函数（风格优化） =============
def save_time(signal, fs, title, path):
    plt.figure(figsize=(6,3))
    if fs and fs>0:
        t=np.arange(len(signal))/fs
        plt.plot(t, signal, color='dodgerblue', lw=1)
        plt.xlabel("时间 (秒)")
    else:
        plt.plot(signal, color='dodgerblue', lw=1)
        plt.xlabel("样本索引")
    plt.ylabel("幅值")
    plt.title(title)
    plt.tight_layout(); plt.savefig(path,dpi=150); plt.close()

def save_psd(signal, fs, title, path, return_arrays=False):
    plt.figure(figsize=(6,3))
    nperseg = min(len(signal), 4096); nperseg = nperseg if nperseg>=8 else len(signal)
    f,P = welch(signal, fs=fs if fs else 1.0, nperseg=nperseg)
    plt.semilogy(f, P, color='crimson', lw=1)
    plt.xlabel("频率 (Hz)" if fs else "归一化频率")
    plt.ylabel("PSD")
    plt.title(title)
    plt.tight_layout(); plt.savefig(path,dpi=150); plt.close()
    if return_arrays: return f,P

def save_spec(signal, fs, title, path):
    plt.figure(figsize=(6,3))
    N = 1024 if len(signal)>=4096 else 512 if len(signal)>=2048 else 256
    plt.specgram(signal, NFFT=N, Fs=fs if fs else 1.0, noverlap=N//2, cmap='viridis')
    plt.xlabel("时间 (秒)" if fs else "样本")
    plt.ylabel("频率 (Hz)" if fs else "cycles/sample")
    plt.title(title)
    plt.tight_layout(); plt.savefig(path,dpi=150); plt.close()
...
def show_save_features_scatter(df, out_prefix):
    ...
    plt.scatter(ok['峭度'], ok['峰值因子'], c='teal', s=15, alpha=0.7)
    plt.xlabel("峭度"); plt.ylabel("峰值因子"); plt.title("峭度 vs 峰值因子")
    ...
    plt.scatter(ok['RMS'], ok['峰值'], c='darkorange', s=15, alpha=0.7)
    plt.xlabel("RMS"); plt.ylabel("峰值"); plt.title("RMS vs 峰值")
...
def show_save_psd_overlay(psd_bank, out_png, max_curves=15):
    ...
    for i,(label,f,P) in enumerate(psd_bank[:max_curves]):
        plt.semilogy(f,P, lw=1, alpha=0.8)
    plt.xlabel("频率 (Hz)"); plt.ylabel("PSD"); plt.title("PSD叠加")
...
def show_save_psd_heatmap(psd_bank, out_png, freq_max=None, n_bins=256):
    ...
    for _,f,P in psd_bank:
        if freq_max is not None: P=P[f<=freq_max]; f=f[f<=freq_max]
        if len(f)!=len(f0) or np.any(f!=f0):
            P=np.interp(f0,f,P)
        X.append(np.log10(P+1e-12))
    X=np.vstack(X)
    if X.shape[1]>n_bins:
    ...
    plt.imshow(X,aspect='auto',origin='lower',extent=[f_disp[0],f_disp[-1],0,X.shape[0]], cmap='magma')
    plt.xlabel("频率 (Hz)"); plt.ylabel("文件索引"); plt.title("PSD热力图")
    plt.tight_layout(); plt.savefig(out_png,dpi=150); plt.show()
...
def show_save_pca(df, out_png):
    ...
    Z=X@Vt[:2].T
    plt.figure(figsize=(5,4))
    plt.scatter(Z[:,0], Z[:,1], c='mediumseagreen', s=20, alpha=0.7)
    plt.xlabel("主成分1"); plt.ylabel("主成分2"); plt.title("统计特征PCA")
    plt.tight_layout(); plt.savefig(out_png,dpi=150); plt.show()
...
def main_cli():
    ...
    limit_files = int(args.get("limit_files","0"))
    ...
    for idx,fp in enumerate(sorted(mat_files)):
        ...
        try:
            ...
            for k,v in md.items():
                kl=k.lower()
                if 'de' in kl and 'time' in kl: signals['DE']=squeeze1d(v)
                elif 'fe' in kl and 'time' in kl: signals['FE']=squeeze1d(v)
                elif 'ba' in kl and 'time' in kl: signals['BA']=squeeze1d(v)
                elif 'rpm' in kl: rpm_arr=squeeze1d(v); labels_name["RPM"]=int(np.median(rpm_arr)) if rpm_arr.size else labels_name.get("RPM")
            if not signals:
                for k in find_numeric_vars(md): signals[k]=squeeze1d(md[k])

            for ch_name, signal_raw in signals.items():
                signal = signal_raw.astype(float)
                fs_plot = fs
                if plot_seconds and (fs_plot and fs_plot>0):
                    signal = signal[: int(min(len(signal), plot_seconds*fs_plot))]
                stats_base = signal.copy()
                if zscore_norm and signal.size>1:
                    m,s = float(np.mean(signal)), float(np.std(signal))
                    if s>0: signal=(signal-m)/s

                signal_plot, fs_down = maybe_downsample(signal, fs_plot, max_points_plot)
                ...
                save_time(signal_plot, fs_down, f"{title_prefix} 时域波形", os.path.join(plots_dir, f"{tag}_时域.png"))
                fP = save_psd(signal_plot, fs_down, f"{title_prefix} PSD功率谱", os.path.join(plots_dir, f"{tag}_PSD.png"), return_arrays=True)
                save_spec(signal_plot, fs_down, f"{title_prefix} 频谱图", os.path.join(plots_dir, f"{tag}_谱图.png"))

                stats = compute_stats(stats_base)
                duration = (len(signal_plot)/fs_down if (fs_down and fs_down>0) else np.nan)
                row = {
                    "相对路径": rel, "文件": os.path.basename(fp), "通道": ch_name,
                    "采样率": fs_down if (fs_down and fs_down>0) else fs,
                    "时长(秒)": duration,
                ...
                with open(meta_path,"w",encoding="utf-8") as f: json.dump(row,f,ensure_ascii=False,indent=2)

                if fP is not None and fs_down and fs_down>0:
                    psd_bank.append((f"{os.path.basename(fp)}-{ch_name}", fP[0], fP[1]))
                    if save_psd_npz:
        ...
        if (idx+1)%50==0: print(f"[INFO] 进度 {idx+1}/{len(mat_files)}")
    ...
    # 综合性图
    show_save_features_scatter(df, os.path.join(aggr_dir,"features_scatter"))
    show_save_psd_overlay(psd_bank, os.path.join(aggr_dir,"psd_overlay.png"), max_curves=15)
    show_save_psd_heatmap(psd_bank, os.path.join(aggr_dir,"psd_heatmap.png"))
    show_save_pca(df, os.path.join(aggr_dir,"pca_scatter.png"))
    print(f"[OK] 综合性图：{aggr_dir}（已显示）")
